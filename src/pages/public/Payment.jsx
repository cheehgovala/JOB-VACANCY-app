  ...
  const handlePay = async (e) => {
    ...
    try {
      ...
      if (response.data.success && response.data.checkout_url) {
        // Redirect to PayChangu — backend /verify handles the return and activates subscription
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md relative z-10 w-full">
        ...
        <div className="glass px-4 py-8 sm:px-10 rounded-2xl">
          ...
          {/* PayChangu provider card */}
          <div className="flex items-center gap-4 p-4 border border-primary-200 bg-primary-50 rounded-xl mb-6 ring-1 ring-primary-300">
            <div className="p-2 bg-white rounded-lg shadow-sm">
              <CreditCard className="h-6 w-6 text-primary-600" />
            </div>
            <div className="flex-1">
              <p className="font-semibold text-gray-900">PayChangu</p>
              <p className="text-xs text-gray-500">Secure mobile & card payments</p>
            </div>
          </div>

          <form onSubmit={handlePay} className="space-y-6">
            <div className="bg-blue-50 border border-blue-100 p-4 rounded-xl text-sm text-blue-800">
              <p>You will be securely redirected to PayChangu to complete your payment. After payment, you will be automatically returned to the app.</p>
            </div>
